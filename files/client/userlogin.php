...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM login WHERE email='$email' AND password='$password' AND (status='$role' OR status = '$role1')";
    $result = mysqli_query($conn, $sql);
    $role2 =mysqli_fetch_assoc($result);
    $status = $role2['status'];
    if (mysqli_num_rows($result) == 1) {
        ...
        header('location:../admin/Board/dashboard.php');
    }
    } else {
